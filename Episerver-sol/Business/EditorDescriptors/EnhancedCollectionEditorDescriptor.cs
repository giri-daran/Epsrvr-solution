...
namespace EpiserverBH.Business.EditorDescriptors
{
    public class EnhancedCollectionEditorDescriptor<T> : CollectionEditorDescriptor<T> where T : new()
    {
        ...
        private static readonly EPiServer.Logging.ILogger Logger = LogManager.GetLogger(typeof(EnhancedCollectionEditorDescriptor<T>));
        ...
        public override void ModifyMetadata(ExtendedMetadata metadata, IEnumerable<Attribute> attributes)
        {
            ...
            metadata.ClientEditingClass = "alloy/editors/EnhancedCollectionEditor";
        ...
        protected virtual IEnumerable<FieldInfo> GetFieldInfo(ExtendedMetadata metadata)
        {
            ...
            try
            {
                ...
                {
                    if (typeof(ContentReference).IsAssignableFrom(property.PropertyType))
                    {
                        fields.Add(new FieldInfo(GetCamelCase(property.Name), IsImage(property)));
                        continue;
                    }

                    if (typeof(Url).IsAssignableFrom(property.PropertyType))
                    {
                        fields.Add(new FieldInfo(GetCamelCase(property.Name), false));
                    }

                    if (typeof(LinkItemCollection).IsAssignableFrom(property.PropertyType))
                    {
                        fields.Add(new FieldInfo(GetCamelCase(property.Name), false));
                    }
                    ...
                    if (url != null && !url.IsEmpty())
                    {
                        if (!listItems.ContainsKey(url.ToString()))
                        {
                            ...
                            else
                            {
                                ...
                                listItems.Add(_urlResolver.Service.GetUrl(content.ContentLink,
                                        ContentLanguage.PreferredCulture.Name,
                                        new UrlResolverArguments { ContextMode = ContextMode.Default }).ToString(), listItem);
                            }
